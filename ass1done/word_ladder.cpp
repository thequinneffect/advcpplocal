...
std::vector<std::string> GetNextRungs(const std::unordered_set<std::string>& lexicon,
                                      seen_map& words_seen,
                                      const std::string& curr_rung,
                                      const std::string& target,
                                      const int& distance);
void BuildSolutionsWithRDFS(seen_map& words_seen,
                            const std::string& curr_word,
                            const std::string& guess,
                            std::string curr_string,
                            std::set<std::string>& solutions);
std::set<std::string>
BuildSolutions(seen_map& words_seen, const std::string& target, const std::string& guess);
...
std::set<std::string> FindWordLadder(const std::unordered_set<std::string>& lexicon,
  ...
  */
  ...
  while (!rungs_q.empty()) {
    ...
    if (max_distance != NOT_SET && rungs_q.front().distance > max_distance)
      break;

    // get all words one letter different from our current word
    auto word = rungs_q.front().word;
    auto next_rungs = GetNextRungs(lexicon, words_seen, word, target, rungs_q.front().distance + 1);
...
void BuildSolutionsWithRDFS(seen_map& words_seen,
  ...
  if (curr_word == guess)
    solutions.insert(curr_string);  // finished solution so add to solutions
